...
describe("loadConfig", () => {
  ...
  it.each([
    ...
    [
      "unique handler instances",
      "duplicate-handler.ts",
      /same handler instance/i,
    ],
    [
      "identifier export names",
      "invalid-export-name.mts",
      /handler export name.*valid javascript identifier/i,
    ],
  ...
  it("ignores default and unrelated named exports", async () => {
    const result = await loadConfig(
      resolve(import.meta.dirname, "../fixtures/configs/ignored-exports.ts"),
    );
  ...
  it("loads indirect named exports", async () => {
    const result = await loadConfig(
      resolve(import.meta.dirname, "../fixtures/configs/indirect-handler.ts"),
    );
