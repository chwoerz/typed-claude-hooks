...
export function mapToolInputNames(names: string[]): [string, string][] {
  const entries = names
    .map((typeName): [string, string] => [
      TOOL_INPUT_ALIASES[typeName] ?? typeName.slice(0, -"Input".length),
      typeName,
    ])
  ...
  entries.forEach(([toolName, typeName], index) => {
    ...
    if (previous?.[0] === toolName) {
      throw new Error(
        `Tool name "${toolName}" maps to both ${previous[1]} and ${typeName}`,
      );
    }
  });
...
export function generateToolInputs(toolsFile: SourceFile): string {
  ...
  const toolDeclarations = toolInputMap.map(([, typeName]) =>
    declarationsByName.get(typeName),
  );
  const toolMapEntries = toolInputMap
